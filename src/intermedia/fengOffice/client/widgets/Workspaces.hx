...
// 		trace("workspaces: "+workspaces);
		// render the template
		var str = haxe.Resource.getString("workspaces");
		var t = new haxe.Template(str);
		var title; 
		if (_curWorkspace != null) title = _curWorkspace.name;
		else title = "Select Workspace";
...
//		trace(output);
	...
	public function refresh(event:Event = null){
	      ...
	      else curId = 0; // the root of the contexts
  
		if (onLoading != null) onLoading(true);

	      _api.listMembers(ServiceTypes.WORKSPACES, curId,cast(_displayContexts), onError);
	}
